...
{
	u64 __iomem *base = (u64 __iomem*)handler->enable_base;
	u64 hwirq_mask = 1ULL << (hwirq % 64);
	int group = hwirq / 64;
	u64 value;

	value = readq(base + group);
    //For single 64 bit access.
...
static void plic_irq_eoi(struct irq_data *d)
{
	...
	reg = (u64 __iomem*)handler->enable_base + (d->hwirq / 64) * sizeof(u64);
	enabled = readq(reg) & BIT(d->hwirq % 64);
...
static void plic_irq_resume(void *data)
{
	...
	for_each_present_cpu(cpu) {
		...
		for (unsigned int i = 0; i < priv->irq_groups; i++) {
			reg = (u64 __iomem *) handler->enable_base + i ; //* sizeof(u32);
			writeq(handler->enable_save[i], reg);
		}
		raw_spin_unlock_irqrestore(&handler->enable_lock, flags);
	}
}
...
static u32 cp100_isolate_pending_irq(int nr_irq_groups, struct plic_handler *handler)
{
	...
	u64 __iomem *enable = handler->enable_base;
	u32 pending_irqs = 0;
	int i, j;
...
static int plic_probe(struct fwnode_handle *fwnode)
{
	...
	priv->irq_groups = DIV_ROUND_UP(nr_irqs, 64);
